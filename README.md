...
- **Data & Services:** Firebase Realtime Database (product catalog) + Firebase Analytics via the modular `firebase` SDK, EmailJS for the contact form — no custom backend API beyond the SSR server itself
- **Tooling:** Angular CLI / `@angular/build` (esbuild), Vitest for unit tests, Firebase Hosting for deploys

Everything bootstraps from `main.ts` — there's no root module, just `bootstrapApplication`. `app.config.ts` wires up the router (view transitions + scroll restoration), `provideZonelessChangeDetection`, and client hydration with event replay (Firebase is initialized outside DI, in `firebase.ts`); `app.config.server.ts` merges in `provideServerRendering()` for the SSR/prerender build. `src/server.ts` is a small Express app that both the Angular CLI's dev/prerender pipeline and the built Node server (`dist/testbakery-website/server/server.mjs`) run through.
...
│   ├── app.config.ts        # Browser providers: router, hydration, HttpClient
